<template>
  <div>
    <vxe-modal
      ...
      <!--标题-->
      <template #title>
        <span style="font-weight: bold;">{{menu.title}}修改操作方法</span>
      </template>
      <!--主体-->
      <template #default>
          ...
          <el-tabs v-model="activeName" type="card">
            <el-tab-pane label="基本信息" name="基本信息">
              ...
              <el-row v-if="form.type == 1">
                ...
                <el-col :span="12">
                  <el-form-item label="操作列宽度" prop="select_type">
...
<script>
import {getPostField, getTableFields, getUrl, updateAction} from '../../sys'
        ...
        if (valid) {
          ...
          updateAction(this.form).then(res => {
            if (res.status == '200') {
              ...
              this.$message({message: '操作出现异常，请联系管理员', type: 'success'})
            }
            this.loading = false
          }).catch(() => {
            this.loading = false
